...
def get_sport_clip(clip_name, verbose=True):
    ...
    clip = sorted(glob(join('data', 'predict', clip_name, '*.png')))
...
def main():
    ...
    net = C3D()
    net.load_state_dict(torch.load('./checkpoint/c3d_baseline.pickle'))
    if torch.cuda.is_available():
        net.cuda()
    net.eval()
    ...
    # read labels
    labels = read_labels_from_file('./data/predict/labels.txt')
